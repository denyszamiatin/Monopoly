...
def test_sort_players(A, B, C, D):
    players = [A, B, C, D]
    assert players_list.sort_players(players, C) == [C, D, A, B]


def test_sort_players_start(A, B, C, D):
    players = [A, B, C, D]
    assert players_list.sort_players(players, A) == [A, B, C, D]


def test_first_player_random(A, B, C, D):
    random.seed(1)
    assert players_list.get_first_player([A, B, C, D]) == D
...
def test_create_players_list(m: unittest.mock.Mock):
    ...
    assert len(players_list.create_players(4)) == 4


@unittest.mock.patch('builtins.input')
def test_create_players_list2(m: unittest.mock.Mock):
    m.side_effect = ['A', 'B', 'C', 'D']
    assert len(players_list.create_players(2)) == 2


@unittest.mock.patch('builtins.input')
def test_create_players(m: unittest.mock.Mock):
    m.side_effect = ['A', 'B', 'C', 'D']
    players = players_list.create_players(4)
